...
Animation provides the illusion of motion: HTML elements change styling over time. Well-designed animations can make your application fun and easier to use, but they are more than just cosmetic. Animation can improve your app and user experience in a number of ways.
...
This guide assumes that you are familiar with the  [Tutorial](tutorial) and [Architecture Overview](guide/architecture) sections. The guide covers the essential Angular animation features and illustrates them through the evolution of a small application that you can run <live-example> live in the browser </live-example>.


The guide begins with simple concepts, and then builds on those to create more complex applications. 
...
When you use CLI to create your app, the root application module named `app.module.ts` is typically placed under the `src/app` folder .
</div>
...
### Step 3: Add the animation metadata property

In the component file, add a metadata property called `animations:` within the `@component` decorator.
This is where you put the trigger that defines an animation that the component uses.

<code-example path="animations/src/app/app.component.ts" title="src/app/app.component.ts" region="decorator">
</code-example>

## Defining a simple transition

We’ll start with an animation example that changes a single HTML element from one state to another.
For example, a button can show as either `open` or `closed`, depending on the user's last action. When the button is in the "Open" state, it's visible and yellow. When it's "Closed" it's transparent and green. In HTML, these attributes are set using ordinary CSS styles such as color and opacity. In Angular, use the `style()` function to specify a set of CSS styles for use with animations. You can collect a set of styles in an animation state, and give the state a name, such as "open" or "closed".
...
In this example, when the `isOpen` expression evaluates to a defined state: `open` or `closed`, it will notify the trigger `openClose` of a state change. It is then up to `openClose` code to handle the state change and kick off a state change animation.
...
In the component file, set the trigger that defines the animation as the value of the animations: property in the @Component decorator.

In the HTML template file, use the trigger name to attach the defined animation to the HTML element to be animated. 
...
In Angular, transiton states can be defined explicitly through the `state()` function, or using the two predefined states: `wildcard *` and `void`.
...
#### Void state
 ...
 You can use the `void` state in a transition to configure transitions for an element that is entering or leaving a page.
...
In the above code, we have apllied `void` state when the HTML element is not attached to a view.
...
<code-example path="animations/src/app/hero-list-page.component.ts" title="src/app/hero-list-page.component.ts" region="increment"></code-example>
...
Normally, when an element within an HTML template has animations disabled using `@.disabled` host binding, animations are disabled on all inner elements as well.  You can't selectively disable multiple animations on a single element.

However, selective child animations can still be run on a disabled parent in one of the following ways:

* A parent animation can use the [query()](#animate-multiple-elements-using-query-and-stagger-functions) function to collect inner elements located in disabled areas of the HTML template. Those elements can still animate.

* A sub-animation can be queried by a parent and then later animated with the `animateChild()` function.
...
The following example demonstrates how we can use `query()` and `stagger()` functions on the entry of an animated element.

*  We use `query()` to look for any element entering or leaving the page. The query specifies elements meeting certain CSS class criteria.

* For each of these elements, we use `style()` to set the same initial style for the element – make it invisible and using transform to move it out of position so that it can slide down into place.

* Use `stagger()` to delay each animation by 30 milliseconds. 

* We animate each element in over 0.5 second using a custom-defined easing curve, simultaneously fading it in and un-transforming it at the same time.
